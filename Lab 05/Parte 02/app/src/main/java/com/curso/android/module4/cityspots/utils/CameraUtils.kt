...
/**
 * Clase sellada que representa los tipos de error posibles al capturar una foto.
 *
 * - CameraClosedUnexpectedly: La cámara se cerró de forma inesperada
 * - HardwareCaptureError: Fallo de hardware durante la captura
 * - FileIOError: Error de E/S (almacenamiento lleno o permisos insuficientes)
 */
sealed class CaptureError {
    data class CameraClosedUnexpectedly(val cause: ImageCaptureException) : CaptureError()
    data class HardwareCaptureError(val cause: ImageCaptureException) : CaptureError()
    data class FileIOError(val cause: ImageCaptureException) : CaptureError()
}
...
class CameraUtils(private val context: Context) {
    ...
    /**
     * Mapea un ImageCaptureException a un CaptureError tipificado
     *
     * ERROR_CAMERA_CLOSED  → CameraClosedUnexpectedly
     * ERROR_CAPTURE_FAILED → HardwareCaptureError
     * ERROR_FILE_IO        → FileIOError
     * Otros                → HardwareCaptureError (fallback)
     */
    fun mapException(exception: ImageCaptureException): CaptureError {
        return when (exception.imageCaptureError) {
            ImageCapture.ERROR_CAMERA_CLOSED -> CaptureError.CameraClosedUnexpectedly(exception)
            ImageCapture.ERROR_CAPTURE_FAILED -> CaptureError.HardwareCaptureError(exception)
            ImageCapture.ERROR_FILE_IO -> CaptureError.FileIOError(exception)
            else -> CaptureError.HardwareCaptureError(exception)
        }
    }
     ...
     * @throws CaptureError si falla la captura (tipificado)
     */
