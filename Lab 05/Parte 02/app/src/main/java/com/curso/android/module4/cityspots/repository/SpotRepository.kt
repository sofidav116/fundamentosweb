...
class SpotRepository(
    ...
    /**
     * Elimina un spot por su ID y borra la foto asociada del almacenamiento
     *
    ...
    suspend fun createSpot(imageCapture: ImageCapture): CreateSpotResult {
        // 1. Capturar la foto — manejar errores tipificados de cámara
        val photoUri = try {
            capturePhoto(imageCapture)
        } catch (e: ImageCaptureException) {
            val captureError = cameraUtils.mapException(e)
            return CreateSpotResult.PhotoCaptureFailed(captureError)
        }
 ...
 * - PhotoCaptureFailed: Error tipificado al capturar la foto (cámara, hardware o E/S)
 */
