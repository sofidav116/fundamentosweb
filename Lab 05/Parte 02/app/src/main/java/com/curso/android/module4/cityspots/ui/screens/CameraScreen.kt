...
fun CameraScreen(
    ...
    // Manejar resultado de captura con mensajes específicos por tipo de error
    LaunchedEffect(captureResult) {
        when (val result = captureResult) {
            ...
            is CreateSpotResult.PhotoCaptureFailed -> {
                // Mostrar mensaje específico según el tipo de error de cámara
            ...
            is CreateSpotResult.NoLocation -> {
                snackbarHostState.showSnackbar("No se pudo obtener tu ubicación GPS.")
                viewModel.clearCaptureResult()
            }
            is CreateSpotResult.InvalidCoordinates -> {
                snackbarHostState.showSnackbar("Coordenadas inválidas: ${result.message}")
                viewModel.clearCaptureResult()
            }
