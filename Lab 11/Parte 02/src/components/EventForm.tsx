...
import {
  generateEventDetailsAction,
  generateEventPosterAction,
  generateEventDescriptionVariantsAction,
...
// =============================================================================
// SELECTOR DE TONO Y VARIANTES DE DESCRIPCIÓN (Parte 2)
// =============================================================================
//
// DescriptionVariantsPanel encapsula toda la lógica de:
// 1. Selector de tono (formal / informal / emocionante)
// 2. Botón "Generar variantes" → llama a generateEventDescriptionVariantsAction
// 3. Indicador de progreso con spinner mientras genera
// 4. Tarjetas de selección para las 3 variantes
// 5. Botón "Aplicar" que rellena el textarea de descripción
// 6. Botón "Regenerar" para obtener nuevas variantes con el mismo tono
// 7. Manejo de errores con mensaje visible
//
// Recibe onApply(description, category, tags) para actualizar el formulario padre.
// =============================================================================

interface DescriptionVariantsPanelProps {
  onApply: (description: string, category: string, tags: string[]) => void;
  onStart: () => void;
  onEnd: () => void;
}

function DescriptionVariantsPanel({ onApply, onStart, onEnd }: DescriptionVariantsPanelProps) {
  const [tone, setTone] = useState<DescriptionTone>('formal');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateEventDescriptionVariantsAction(title, tone);

      if (result.success && result.data) {
        setVariants(result.data.variants);
        setGeneratedCategory(result.data.category);
        setGeneratedTags(result.data.tags);
        toast({
          title: '✨ Variantes generadas',
          description: 'Elige tu descripción favorita y presiona Aplicar.',
        });
      } else {
        setError(result.error || 'No se pudo generar el contenido');
        toast({
          title: 'Error',
  ...
  const handleApply = () => {
    ...
    onApply(variants[selectedIndex].description, generatedCategory, generatedTags);
    toast({
      title: '✅ Descripción aplicada',
      description: 'La variante seleccionada se aplicó al formulario.',
    });
  };
    ...
    <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
      ...
      {isGenerating && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>Generando 3 variantes con tono {toneOptions.find(t => t.value === tone)?.label.toLowerCase()}…</span>
        </div>
      )}
      ...
      {variants.length > 0 && !isGenerating && (
        <div className="space-y-2">
              ...
              <div className="flex items-start gap-2">
                ...
                <CheckCircle2
                  className={`mt-0.5 h-4 w-4 shrink-0 transition-colors ${
                    selectedIndex === index ? 'text-primary' : 'text-muted-foreground/30'
                  }`}
                />
                <span className="leading-relaxed">{variant.description}</span>
              </div>
            </button>
          ))}
          ...
          >
            Aplicar descripción seleccionada
          </Button>
        </div>
      )}
    </div>
      ...
      <CardContent>
        ...
        <form action={formAction} className="space-y-6">
          <div className="space-y-4">
            ...
            {/* Panel de variantes IA — solo en modo creación */}
            {!isEditing && (
              <DescriptionVariantsPanel
                onStart={() => setIsAiGenerating(true)}
                onEnd={() => setIsAiGenerating(false)}
                onApply={(description, category, tags) => {
                  // Rellena el textarea de descripción
                  const descInput = document.getElementById('description') as HTMLTextAreaElement;
